<?php $__env->startSection('title', 'رسائل التواصل - لوحة التحكم'); ?>
  ...
  <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
    <?php $__currentLoopData = $statusLabels; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $status => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
      <div class="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm text-right">
        <div class="w-10 h-10 bg-gradient-to-br <?php echo e($statusGradients[$status]); ?> rounded-lg flex items-center justify-center text-white mb-2">
          <i data-lucide="mail-open" class="w-5 h-5"></i>
        </div>
        <div class="text-2xl font-black text-gray-800 tabular-nums">
          <?php echo e($messages->where('status', $status)->count()); ?>
      ...
      <table class="w-full text-right border-collapse" id="messages-table">
        ...
        <tbody class="divide-y divide-gray-50 text-sm">
          <?php $__empty_1 = true; $__currentLoopData = $messages; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $msg): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
            <tr
              class="message-row hover:bg-gray-50 transition-colors <?php echo e($msg->status === 'unread' ? 'font-bold bg-amber-50/20' : ''); ?>"
            ...
            >
              <td class="p-4 text-gray-800"><?php echo e($msg->name); ?></td>
              <td class="p-4 text-gray-600 font-mono" dir="ltr"><?php echo e($msg->email); ?></td>
              <td class="p-4 text-gray-700"><?php echo e($msg->subject); ?></td>
              <td class="p-4 text-gray-400 max-w-xs truncate"><?php echo e($msg->message); ?></td>
              <td class="p-4 text-center">
                <span class="px-2.5 py-1 rounded-full text-xs font-bold border <?php echo e($statusColors[$msg->status] ?? ''); ?>">
                  <?php echo e($statusLabels[$msg->status] ?? $msg->status); ?>

                </span>
              </td>
              <td class="p-4 text-center text-gray-500 text-xs font-mono">
                <?php echo e($msg->created_at ? $msg->created_at->format('Y-m-d H:i') : 'لا يوجد'); ?>

              </td>
              <td class="p-4">
                <div class="flex items-center justify-center gap-2">
                  <button
                    onclick="openResponseModal(<?php echo e($msg->id); ?>, '<?php echo e(addslashes($msg->name)); ?>', '<?php echo e($msg->email); ?>', '<?php echo e(addslashes($msg->subject)); ?>', '<?php echo e(addslashes($msg->message)); ?>', '<?php echo e($msg->status); ?>', '<?php echo e(addslashes($msg->reply)); ?>')"
                    class="p-2 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100 transition-colors cursor-pointer"
                    title="عرض / رد"
                  >
                    <i data-lucide="message-square" class="w-4 h-4"></i>
                  </button>
                  <form action="<?php echo e(route('admin.messages.destroy', $msg->id)); ?>" method="POST" onsubmit="return confirm('هل تريد حذف هذه الرسالة؟');">
                    <?php echo csrf_field(); ?>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\laravel-hospital-website-development\hospital-backend\resources\views/admin/messages/index.blade.php ENDPATH**/ ?>