<?php $__env->startSection('title', 'أقسام العرض (Screens) - لوحة التحكم'); ?>
...
<?php $__env->startSection('content'); ?>
      ...
      <table class="w-full text-right border-collapse" id="screens-table">
        ...
        <tbody class="divide-y divide-gray-50 text-sm">
          <?php $__empty_1 = true; $__currentLoopData = $screens; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $screen): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
            <tr
              class="screen-row hover:bg-gray-50 transition-colors <?php echo e(!$screen->enabled ? 'opacity-60 bg-gray-50/40' : ''); ?>"
              data-name="<?php echo e($screen->name); ?>"
              data-component="<?php echo e($screen->component); ?>"
            >
              <td class="p-4 text-center font-bold text-primary-700 font-mono tabular-nums">
                <div class="flex items-center justify-center gap-1.5">
                  <form action="<?php echo e(route('admin.screens.up', $screen->id)); ?>" method="POST" class="inline">
                    <?php echo csrf_field(); ?>
                  ...
                  <span class="w-6 text-center text-sm font-bold text-gray-700"><?php echo e($screen->order); ?></span>
                  <form action="<?php echo e(route('admin.screens.down', $screen->id)); ?>" method="POST" class="inline">
                    <?php echo csrf_field(); ?>
              ...
              <td class="p-4 font-bold text-gray-800"><?php echo e($screen->name); ?></td>
              <td class="p-4 text-gray-500 font-mono"><?php echo e($screen->component); ?></td>
              <td class="p-4">
                <div class="flex items-center justify-center text-gray-600">
                  <i data-lucide="<?php echo e($screen->icon); ?>" class="w-5 h-5"></i>
                </div>
              </td>
              <td class="p-4 text-center">
                <!-- Active Toggle Button -->
                <form action="<?php echo e(route('admin.screens.update', $screen->id)); ?>" method="POST" class="inline">
                  <?php echo csrf_field(); ?>
                  <?php echo method_field('PUT'); ?>
                  <input type="hidden" name="name" value="<?php echo e($screen->name); ?>" />
                  <input type="hidden" name="component" value="<?php echo e($screen->component); ?>" />
                  <input type="hidden" name="order" value="<?php echo e($screen->order); ?>" />
                  <input type="hidden" name="icon" value="<?php echo e($screen->icon); ?>" />
                  <?php if($screen->enabled): ?>
              ...
              <td class="p-4">
                <div class="flex items-center justify-center gap-2">
                  <button
                    onclick="openEditModal(<?php echo e($screen->id); ?>, '<?php echo e(addslashes($screen->name)); ?>', '<?php echo e(addslashes($screen->component)); ?>', '<?php echo e($screen->order); ?>', '<?php echo e($screen->icon); ?>', <?php echo e($screen->enabled ? 'true' : 'false'); ?>)"
                  ...
                  <form action="<?php echo e(route('admin.screens.destroy', $screen->id)); ?>" method="POST" onsubmit="return confirm('حذف هذا القسم قد يخفي بياناته تماماً من واجهة المستخدم. هل تريد الحذف بالفعل؟');">
                    <?php echo csrf_field(); ?>
  ...
  <div id="create-modal" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="relative bg-white rounded-3xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto animate-scale-in text-right">
      ...
      <form action="<?php echo e(route('admin.screens.store')); ?>" method="POST" class="p-6 space-y-4 text-gray-700">
        <?php echo csrf_field(); ?>
        <div>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\laravel-hospital-website-development\resources\views\admin\screens\index.blade.php ENDPATH**/ ?>