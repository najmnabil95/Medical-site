<?php $__env->startSection('title', 'آراء المرضى - لوحة التحكم'); ?>
...
<?php $__env->startSection('content'); ?>
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" id="testimonials-grid">
    <?php $__empty_1 = true; $__currentLoopData = $testimonials; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $test): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
      ...
      ?>
      <div
        class="testimonial-card border rounded-3xl p-5 text-right flex flex-col justify-between hover:shadow-xl transition-all <?php echo e($colorClass); ?>"
        data-name="<?php echo e($test->name); ?>"
        data-role="<?php echo e($test->role); ?>"
        data-text="<?php echo e($test->text); ?>"
      >
        <div>
          <!-- Patient header -->
          <div class="flex items-center justify-start gap-3 mb-4">
            <div class="w-12 h-12 rounded-full overflow-hidden border-2 border-white shadow-sm shrink-0">
              <img src="<?php echo e($test->avatar); ?>" alt="<?php echo e($test->name); ?>" class="w-full h-full object-cover" onerror="this.src='https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=150&q=80'" />
            </div>
            <div class="text-right">
              <h4 class="font-bold text-sm block leading-tight"><?php echo e($test->name); ?></h4>
              <span class="text-[10px] text-gray-400 font-bold"><?php echo e($test->role); ?></span>
            </div>
          </div>
        ...
        <div class="flex items-center gap-2 pt-4 mt-6 border-t border-black/5">
          <button
            onclick="openEditModal(<?php echo e($test->id); ?>, '<?php echo e(addslashes($test->name)); ?>', '<?php echo e(addslashes($test->role)); ?>', '<?php echo e(addslashes($test->text)); ?>', '<?php echo e($test->rating); ?>', '<?php echo e($test->color); ?>')"
          ...
          <form action="<?php echo e(route('admin.testimonials.destroy', $test->id)); ?>" method="POST" class="flex-1" onsubmit="return confirm('هل تريد إزالة هذا الرأي نهائياً؟');">
            <?php echo csrf_field(); ?>
  ...
  <div id="create-modal" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="relative bg-white rounded-3xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto animate-scale-in text-right">
      ...
      <form action="<?php echo e(route('admin.testimonials.store')); ?>" method="POST" enctype="multipart/form-data" class="p-6 space-y-4 text-gray-700">
        <?php echo csrf_field(); ?>
        ...
        <div class="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select name="color" required class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-primary-500">
              <?php $__currentLoopData = $colorsList; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $val => $lbl): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\laravel-hospital-website-development\hospital-backend\resources\views/admin/testimonials/index.blade.php ENDPATH**/ ?>