...
@section('title', 'شركاء النجاح - لوحة التحكم')
...
@section('content')
    ...
    @forelse($partners as $ptnr)
      ...
      >
        <div>
          ...
          <h3 class="text-base font-bold text-gray-800 mb-1">{{ $ptnr->name }}</h3>
          <p class="text-gray-400 text-xs font-semibold">{{ $ptnr->sub }}</p>
        </div>

        <div class="flex items-center gap-2 pt-3 border-t border-gray-50 mt-6">
          <button
            onclick="openEditModal({{ $ptnr->id }}, '{{ addslashes($ptnr->name) }}', '{{ addslashes($ptnr->sub) }}', '{{ $ptnr->emoji }}')"
          ...
          <form action="{{ route('admin.partners.destroy', $ptnr->id) }}" method="POST" class="flex-1" onsubmit="return confirm('هل تريد إزالة هذا الشريك نهائياً؟');">
            @csrf
            @method('DELETE')
            <button
              type="submit"
  ...
  <div id="create-modal" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="relative bg-white rounded-3xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto animate-scale-in text-right">
      ...
      <form action="{{ route('admin.partners.store') }}" method="POST" class="p-6 space-y-4 text-gray-700">
        @csrf
        <div>
