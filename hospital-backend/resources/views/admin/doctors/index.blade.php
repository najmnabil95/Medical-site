      ...
      >
        ...
        <div class="p-5 text-right flex flex-col justify-between h-full">
          ...
          <div class="flex items-center gap-2 pt-3 border-t border-gray-50">
            <button
              onclick="openEditModal({{ $doc->id }}, '{{ addslashes($doc->name) }}', '{{ addslashes($doc->specialty) }}', '{{ addslashes($doc->department) }}', '{{ $doc->rating }}', '{{ preg_replace('/[^0-9]/', '', $doc->experience) }}', '{{ preg_replace('/[^0-9]/', '', $doc->patients) }}', '{{ $doc->gradient }}', {{ $doc->active ? 'true' : 'false' }})"
