...
@section('title', 'شهادات الاعتماد - لوحة التحكم')
...
@section('content')
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5" id="certs-grid">
    @forelse($certifications as $cert)
      <div
        ...
        data-name="{{ $cert->name }}"
        data-fullname="{{ $cert->full_name }}"
        data-desc="{{ $cert->desc }}"
      >
        <div>
          <div class="flex items-start justify-between mb-4">
            <div class="w-12 h-12 rounded-xl flex items-center justify-center border {{ $cert->bg }} {{ $cert->border }} {{ $cert->color }}">
              <i data-lucide="{{ $cert->icon }}" class="w-6 h-6"></i>
            </div>
          ...
          <h3 class="text-base font-bold text-gray-800 mb-1">{{ $cert->name }}</h3>
          <h4 class="text-xs font-bold text-gray-400 mb-3">{{ $cert->full_name }}</h4>
          <p class="text-gray-500 text-xs leading-relaxed mb-6">{{ $cert->desc }}</p>
        </div>

        <div class="flex items-center gap-2 pt-3 border-t border-gray-50 mt-4">
          <button
            onclick="openEditModal({{ $cert->id }}, '{{ addslashes($cert->name) }}', '{{ addslashes($cert->full_name) }}', '{{ $cert->icon }}', '{{ addslashes($cert->desc) }}', '{{ $cert->year }}', '{{ $cert->color }}', '{{ $cert->border }}', '{{ $cert->bg }}')"
          ...
          <form action="{{ route('admin.certifications.destroy', $cert->id) }}" method="POST" class="flex-1" onsubmit="return confirm('هل تريد حذف شهادة الاعتماد هذه؟');">
            @csrf
            @method('DELETE')
            <button
              type="submit"
  ...
  <div id="create-modal" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="relative bg-white rounded-3xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto animate-scale-in text-right">
      ...
      <form action="{{ route('admin.certifications.store') }}" method="POST" class="p-6 space-y-4 text-gray-700">
        @csrf
        ...
        <div class="grid grid-cols-3 gap-4">
          ...
          <div>
            ...
            <select id="create-style-select" onchange="applyPredefinedStyle('create')" class="w-full px-4 py-3 bg-blue-50 border border-blue-200 text-blue-700 font-bold rounded-xl text-sm focus:outline-none">
              @foreach($stylesList as $val => $style)
                <option value="{{ $val }}">{{ $style['lbl'] }}</option>
              @endforeach
            </select>
          </div>
        </div>
