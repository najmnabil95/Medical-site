...
@section('title', 'إدارة الأقسام الطبية - لوحة التحكم')
...
@section('content')
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5" id="departments-grid">
    @forelse($departments as $dept)
      ...
      @endphp
      <div
        class="dept-card bg-white rounded-2xl border border-gray-100 p-5 text-right flex flex-col justify-between hover:shadow-xl transition-all {{ !$dept->active ? 'opacity-60' : '' }}"
        data-name="{{ $dept->name }}"
        data-desc="{{ $dept->desc }}"
      >
        <div>
          <div class="flex items-start justify-between mb-4">
            <div class="w-12 h-12 rounded-xl flex items-center justify-center border {{ $colorClass }}">
              <i data-lucide="{{ $dept->icon }}" class="w-6 h-6"></i>
            </div>
            
            <!-- Active Toggle Button -->
            <form action="{{ route('admin.departments.update', $dept->id) }}" method="POST" class="inline">
              @csrf
              @method('PUT')
              <input type="hidden" name="name" value="{{ $dept->name }}" />
              <input type="hidden" name="icon" value="{{ $dept->icon }}" />
              <input type="hidden" name="desc" value="{{ $dept->desc }}" />
          ...
          <p class="text-gray-500 text-xs leading-relaxed mb-6">{{ $dept->desc }}</p>
        </div>
        ...
        <div class="flex items-center gap-2 pt-3 border-t border-gray-50">
          ...
          <form action="{{ route('admin.departments.destroy', $dept->id) }}" method="POST" class="flex-1" onsubmit="return confirm('حذف هذا القسم قد يؤثر على الأطباء والحجوزات المرتبطة به. هل تريد الحذف بالفعل؟');">
            @csrf
            @method('DELETE')
            <button
              type="submit"
  ...
  <div id="create-modal" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="relative bg-white rounded-3xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto animate-scale-in text-right">
      ...
      <form action="{{ route('admin.departments.store') }}" method="POST" class="p-6 space-y-4 text-gray-700">
        @csrf
        <div>
