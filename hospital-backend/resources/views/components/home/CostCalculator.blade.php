  ...
  <div class="max-w-7xl mx-auto px-4 relative">
    ...
    <div class="grid lg:grid-cols-3 gap-6 animate-fade-in-up">
      ...
      <div class="lg:col-span-2 space-y-3">
        @foreach($groupedPriceItems as $category => $items)
          @php
            $catId = 'cat-' . Str::slug($category);
          @endphp
          <div class="bg-white rounded-2xl border border-gray-100 overflow-hidden">
            <button
              onclick="toggleCalculatorCategory('{{ $catId }}')"
            ...
            >
              ...
              <i data-lucide="chevron-down" id="arrow-{{ $catId }}" class="text-gray-400 transition-transform"></i>
            </button>

            <div id="content-{{ $catId }}" class="hidden border-t border-gray-100 p-4 grid grid-cols-1 md:grid-cols-2 gap-2">
              @foreach($items as $item)
                <button
                  id="calc-item-btn-{{ $item->id }}"
                  onclick="toggleCalculatorItem({{ $item->id }}, '{{ addslashes($item->service) }}', {{ $item->price }})"
                ...
                >
                  <div class="flex-1">
                    ...
                    <p class="text-primary-600 font-bold text-sm mt-1">{{ $item->price }} {{ $item->currency ?? 'ر.س' }}</p>
                  </div>
                  <div id="calc-item-indicator-{{ $item->id }}" class="w-7 h-7 rounded-lg flex items-center justify-center shrink-0 bg-gray-100 text-gray-400 transition-all">
