...
<section id="home" class="relative min-h-[82svh] overflow-hidden bg-[#081f24] text-white">
  ...
  <div id="hero-slides-wrapper">
    @foreach($heroSlides as $index => $slide)
      <div
        class="hero-slide absolute inset-0 transition-opacity duration-1000 {{ $index === 0 ? 'opacity-100 z-10' : 'opacity-0 z-0' }}"
        data-slide-index="{{ $index }}"
      >
        <img src="{{ $slide['image'] }}" alt="فريق طبي داخل مستشفى الشفاء" class="h-full w-full object-cover" />
    ...
    <div class="flex flex-col justify-center">
      ...
      <div id="hero-content-area" class="max-w-3xl min-h-[220px]">
        <div class="mb-5 inline-flex items-center gap-3 rounded-[8px] border border-white/15 bg-white/10 px-4 py-2 text-sm font-bold backdrop-blur-md">
          ...
          <span id="hero-slide-eyebrow">{{ $heroSlides[0]['eyebrow'] }}</span>
        </div>
      ...
      <div class="mt-8 flex flex-wrap gap-3 z-20">
        ...
        >
          ...
          <span>الطوارئ {{ $settings->emergency ?? '920012345' }}</span>
        </a>
      </div>

      <!-- Care Path -->
      <div class="mt-10 grid max-w-3xl grid-cols-2 gap-3 md:grid-cols-4 z-20">
        @foreach($carePath as $index => $item)
          <div class="rounded-[8px] border border-white/10 bg-white/[0.08] p-4 backdrop-blur-md">
            <div class="mb-4 flex items-center justify-between">
              <i data-lucide="{{ $item['icon'] }}" class="text-[#76e4d2] w-5 h-5"></i>
              <span class="font-mono text-xs text-white/30">0{{ $index + 1 }}</span>
            </div>
            <p class="text-sm font-black">{{ $item['label'] }}</p>
            <p class="mt-1 text-xs text-white/50">{{ $item['value'] }}</p>
          </div>
        @endforeach
      </div>
    </div>
      ...
      <div class="w-full max-w-[520px] rounded-[8px] border border-white/10 bg-[#07181d]/70 p-4 shadow-2xl shadow-black/35 backdrop-blur-xl">
        ...
        <div class="grid grid-cols-[1fr_auto] items-end gap-4 rounded-[8px] bg-[#f8faf7] p-4 text-[#102a2d]">
          <div>
            ...
            <p id="hero-slide-metric" class="mt-2 text-4xl font-black">{{ $heroSlides[0]['metric'] }}</p>
            <p id="hero-slide-metric-label" class="mt-1 text-sm text-slate-600">{{ $heroSlides[0]['metricLabel'] }}</p>
          </div>
    ...
    <div class="flex items-center gap-2">
      @foreach($heroSlides as $index => $slide)
        <button
          ...
          class="hero-dot-indicator h-2.5 rounded-full transition-all {{ $index === 0 ? 'w-10 bg-[#f6715b]' : 'w-2.5 bg-white/30 hover:bg-white/60' }}"
...
<script>
  const heroSlidesData = @json($heroSlides);
