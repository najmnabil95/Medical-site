...
@section('content')
  ...
  <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
    @foreach($statusLabels as $status => $label)
      <div class="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm text-right relative overflow-hidden">
        <div class="w-10 h-10 bg-gradient-to-br {{ $statusBgGradients[$status] }} rounded-lg flex items-center justify-center text-white mb-2">
  ...
  <div class="flex gap-2 mb-6 border-b border-gray-100 pb-2 overflow-x-auto">
    <a href="{{ route('admin.appointments.index', ['tab' => 'today']) }}" 
    ...
    <a href="{{ route('admin.appointments.index', ['tab' => 'pending']) }}" 
       class="px-4 py-2 text-sm font-bold rounded-xl transition-all whitespace-nowrap {{ $tab === 'pending' ? 'bg-amber-50 text-amber-700' : 'text-gray-500 hover:bg-gray-50' }}">
      قيد الانتظار
    </a>
    <a href="{{ route('admin.appointments.index', ['tab' => 'all']) }}" 
       class="px-4 py-2 text-sm font-bold rounded-xl transition-all whitespace-nowrap {{ $tab === 'all' ? 'bg-gray-100 text-gray-800' : 'text-gray-500 hover:bg-gray-50' }}">
      جميع الحجوزات
    </a>
  </div>
        ...
        <tbody class="divide-y divide-gray-50 text-sm">
          @forelse($appointments as $appt)
            <tr
              ...
              data-dept="{{ $appt->department }}"
            >
              <td class="p-4 font-bold text-gray-800">{{ $appt->patient_name }}</td>
              <td class="p-4 text-gray-600 font-mono" dir="ltr">{{ $appt->phone }}</td>
              <td class="p-4 text-gray-600">{{ $appt->department }}</td>
              <td class="p-4 text-primary-700 font-medium">{{ $appt->doctor }}</td>
              ...
              <td class="p-4">
                <div class="flex items-center justify-center gap-2">
                  ...
                  @if($appt->status === 'pending')
                    <form action="{{ route('admin.appointments.updateStatus', $appt->id) }}" method="POST" class="inline-block">
                      @csrf
                      @method('PUT')
                  ...
                  <!-- Edit -->
                  <button
                    onclick="openEditModal({{ $appt->id }}, '{{ addslashes($appt->patient_name) }}', '{{ $appt->phone }}', '{{ addslashes($appt->department) }}', '{{ addslashes($appt->doctor) }}', '{{ $appt->date->format('Y-m-d') }}', '{{ $appt->time }}', '{{ $appt->status }}', '{{ addslashes($appt->type) }}', '{{ addslashes($appt->notes) }}', '{{ $appt->offer_id }}')"
  ...
  <div id="create-modal" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="relative bg-white rounded-3xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto animate-scale-in text-right">
      ...
      <form action="{{ route('admin.appointments.store') }}" method="POST" class="p-6 space-y-4 text-gray-700">
        @csrf
