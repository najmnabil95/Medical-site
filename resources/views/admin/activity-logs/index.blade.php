...
<div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 text-right">
  ...
  @if($logs->total() > 0)
    <form action="{{ route('admin.activity-logs.clearAll') }}" method="POST" onsubmit="return confirm('هل أنت متأكد من رغبتك في حذف جميع سجلات النشاطات؟ لا يمكن التراجع عن هذا الإجراء.');">
      @csrf
      @method('DELETE')
      <button type="submit" 
...
<div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden text-right">
  <div class="px-6 py-4 border-b border-gray-50 flex items-center justify-between">
    ...
    <span class="text-xs text-gray-500 font-bold bg-gray-50 px-3 py-1 rounded-full">إجمالي العمليات: {{ $logs->total() }}</span>
  </div>
      ...
      <tbody class="divide-y divide-gray-50">
        @forelse($logs as $log)
          <tr class="hover:bg-gray-50/30 transition-colors">
            ...
            <td class="px-6 py-4">
              <span class="text-sm font-semibold text-gray-800 leading-relaxed">{{ $log->action }}</span>
            </td>
            ...
            <td class="px-6 py-4">
              ...
              @endphp
              <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-bold {{ $typeConfig['class'] }}">
                {{ $typeConfig['label'] }}
              </span>
            </td>
            ...
            <td class="px-6 py-4">
              <div class="flex items-center gap-2">
                <div class="w-7 h-7 rounded-lg bg-gray-100 flex items-center justify-center text-gray-600 font-bold text-xs shrink-0">
                  {{ mb_substr($log->user, 0, 2) }}
                </div>
                <span class="text-sm font-semibold text-gray-700">{{ $log->user }}</span>
              </div>
            </td>
            ...
            <td class="px-6 py-4">
              @if($log->timestamp)
                <p class="text-sm font-semibold text-gray-800">
                  {{ \Carbon\Carbon::parse($log->timestamp)->locale('ar')->isoFormat('YYYY-MM-DD') }}
                </p>
