...
@section('title', 'شركات التأمين - لوحة التحكم')
...
@section('content')
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5" id="insurances-grid">
    @forelse($insurances as $ins)
      <div
        class="insurance-card bg-white rounded-2xl border border-gray-100 p-5 text-right flex flex-col justify-between hover:shadow-lg transition-all {{ !$ins->active ? 'opacity-60' : '' }}"
        data-name="{{ $ins->name }}"
        data-abbr="{{ $ins->abbr }}"
      >
        <div>
          <div class="flex items-start justify-between mb-4">
            ...
            <form action="{{ route('admin.insurances.update', $ins->id) }}" method="POST" class="inline">
              @csrf
              @method('PUT')
              <input type="hidden" name="name" value="{{ $ins->name }}" />
              <input type="hidden" name="abbr" value="{{ $ins->abbr }}" />
              @if($ins->active)
          ...
          <h3 class="text-base font-bold text-gray-800 mb-1">{{ $ins->name }}</h3>
          <p class="text-gray-400 text-xs font-bold font-mono">ID / ABBR: {{ $ins->abbr }}</p>
        </div>

        <div class="flex items-center gap-2 pt-3 border-t border-gray-50 mt-5">
          <button
            onclick="openEditModal({{ $ins->id }}, '{{ addslashes($ins->name) }}', '{{ $ins->abbr }}', {{ $ins->active ? 'true' : 'false' }})"
          ...
          <form action="{{ route('admin.insurances.destroy', $ins->id) }}" method="POST" class="flex-1" onsubmit="return confirm('هل تريد إزالة شركة التأمين نهائياً؟');">
            @csrf
            @method('DELETE')
            <button
              type="submit"
  ...
  <div id="create-modal" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="relative bg-white rounded-3xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto animate-scale-in text-right">
      ...
      <form action="{{ route('admin.insurances.store') }}" method="POST" class="p-6 space-y-4 text-gray-700">
        @csrf
        <div>
