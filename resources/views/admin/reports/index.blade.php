...
@section('title', 'التقارير والإحصائيات - لوحة التحكم')
...
<div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4 text-right">
  ...
  <div class="flex items-center gap-2 self-start md:self-auto">
    ...
    <div class="flex gap-1 bg-gray-50 rounded-xl p-1 border border-gray-100">
      @foreach([7 => '7 أيام', 30 => '30 يوم', 90 => '3 أشهر', 365 => 'سنة'] as $days => $label)
        <a href="{{ route('admin.reports.index', ['period' => $days]) }}"
          class="px-4 py-2 rounded-lg text-xs font-bold transition-all {{ (int)$period === $days ? 'bg-primary-500 text-white shadow-md shadow-primary-500/20' : 'text-gray-500 hover:bg-gray-100' }}">
          {{ $label }}
        </a>
      @endforeach
    </div>
...
<div class="grid grid-cols-2 lg:grid-cols-4 gap-5 mb-7">
  ...
  <div class="group relative bg-white rounded-2xl p-5 border border-gray-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 overflow-hidden">
    ...
    <p class="text-3xl font-black text-gray-900 tabular-nums leading-none mb-1">{{ $periodAppointments }}</p>
    <p class="text-xs text-gray-400 font-semibold">حجوزات خلال {{ $period }} يوم</p>
  </div>
  ...
  <div class="group relative bg-white rounded-2xl p-5 border border-gray-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 overflow-hidden">
    ...
    <p class="text-3xl font-black text-gray-900 tabular-nums leading-none mb-1">{{ $completedAppointments }}</p>
  ...
  <div class="group relative bg-white rounded-2xl p-5 border border-gray-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 overflow-hidden">
    ...
    <p class="text-3xl font-black text-gray-900 tabular-nums leading-none mb-1">{{ $pendingAppointments }}</p>
  ...
  <div class="group relative bg-white rounded-2xl p-5 border border-gray-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 overflow-hidden">
    ...
    <p class="text-3xl font-black text-gray-900 tabular-nums leading-none mb-1">{{ $cancelledAppointments }}</p>
  ...
  <div class="lg:col-span-2 bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
    <div class="flex items-center justify-between mb-6 text-right">
      <div>
        ...
        <p class="text-xs text-gray-400 mt-0.5">آخر {{ $period }} يوم</p>
      </div>
    ...
    <div class="mt-4 space-y-2">
      ...
      <div class="flex items-center justify-between text-sm">
        <div class="flex items-center gap-2">
          <span class="w-3 h-3 rounded-full" @style(['background-color: ' . ($statusColors[$status] ?? '#94a3b8')])></span>
          <span class="text-gray-600 font-medium">{{ $statusLabels[$status] ?? $status }}</span>
        </div>
    ...
    <div class="space-y-4">
      ...
      <div>
        <div class="flex justify-between items-center mb-1.5">
          ...
          <span class="text-xs font-black text-primary-600">{{ $count }} حجز</span>
        </div>
        <div class="h-2.5 bg-gray-100 rounded-full overflow-hidden">
          <div class="h-full bg-linear-to-l from-violet-400 to-violet-600 rounded-full transition-all duration-700" @style(['width: ' . $percentage . '%'])></div>
        </div>
      </div>
      @empty
    ...
    <div class="overflow-x-auto">
      <table class="w-full" dir="rtl">
        ...
        <tbody class="divide-y divide-gray-50">
          @forelse($doctorPerformance as $docName => $stats)
          @php
            $completionRate = $stats['total'] > 0 ? round(($stats['completed'] / $stats['total']) * 100) : 0;
          @endphp
          <tr class="hover:bg-gray-50/50 transition-colors">
            <td class="px-6 py-3.5">
              <span class="font-bold text-gray-800 text-sm">{{ $docName }}</span>
            </td>
            <td class="px-4 py-3.5 text-center">
              <span class="font-bold text-gray-700 text-sm">{{ $stats['total'] }}</span>
            </td>
            <td class="px-4 py-3.5 text-center">
              <span class="text-emerald-600 font-bold text-sm">{{ $stats['completed'] }}</span>
            </td>
            <td class="px-4 py-3.5 text-center">
              <span class="text-rose-600 font-bold text-sm">{{ $stats['cancelled'] }}</span>
            </td>
            <td class="px-4 py-3.5 text-center">
              <div class="flex items-center justify-center gap-2">
                ...
                <span class="text-xs font-bold text-gray-600">{{ $completionRate }}%</span>
              </div>
            </td>
          </tr>
          @empty
...
<div class="grid lg:grid-cols-3 gap-6">
  ...
  <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
    ...
    <div class="space-y-3">
      @forelse($peakHours as $time => $count)
      @php
        $maxPeak = $peakHours->max();
        $peakPct = $maxPeak > 0 ? round(($count / $maxPeak) * 100) : 0;
      @endphp
      <div class="flex items-center gap-3">
        <span class="text-sm font-mono font-bold text-gray-600 w-14 text-center bg-gray-50 py-1 rounded-lg">{{ $time }}</span>
        <div class="flex-1 h-2 bg-gray-100 rounded-full overflow-hidden">
          <div class="h-full bg-linear-to-l from-amber-400 to-orange-500 rounded-full" @style(['width: ' . $peakPct . '%'])></div>
        </div>
        <span class="text-xs font-bold text-gray-500 w-8 text-left">{{ $count }}</span>
      </div>
      @empty
  ...
  <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
    ...
    <div class="space-y-2">
      ...
      <div class="flex items-center justify-between text-sm">
        <span class="text-gray-600 font-medium">{{ $typeLabels[$type] ?? $type }}</span>
        <span class="font-bold text-gray-800 bg-gray-50 px-2.5 py-0.5 rounded-lg">{{ $count }}</span>
      </div>
      @endforeach
    </div>
  </div>
  ...
  <div class="bg-linear-to-br from-slate-700 via-slate-800 to-slate-900 rounded-2xl p-6 text-white shadow-lg relative overflow-hidden">
    ...
    <div class="relative">
      ...
      <div class="space-y-5">
        <div>
          <p class="text-4xl font-black tabular-nums">{{ $totalMessages }}</p>
        ...
        <div class="flex items-center justify-between">
          <div>
            <p class="text-2xl font-black text-amber-400 tabular-nums">{{ $newMessages }}</p>
            <p class="text-slate-400 text-xs mt-0.5">رسائل جديدة</p>
          </div>
          <div>
            <p class="text-2xl font-black tabular-nums">{{ $totalMessages - $newMessages }}</p>
