...
@section('title', 'إدارة باقات الكشف - لوحة التحكم')
...
@section('content')
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" id="packages-grid">
    @forelse($packages as $pkg)
      <div
        class="package-card bg-white rounded-3xl border border-gray-100 overflow-hidden hover:shadow-2xl transition-all relative flex flex-col justify-between {{ !$pkg->active ? 'opacity-60' : '' }}"
        data-name="{{ $pkg->name }}"
        data-name-en="{{ $pkg->name_en }}"
      >
        @if($pkg->popular)
        ...
        <div>
          ...
          <div class="h-28 bg-gradient-to-br {{ $pkg->gradient }} p-5 text-white flex items-center gap-3 relative">
            <div class="w-12 h-12 bg-white/20 backdrop-blur-md rounded-2xl flex items-center justify-center">
              <i data-lucide="{{ $pkg->icon }}" class="w-6 h-6"></i>
            </div>
            <div class="text-right">
              <h3 class="font-bold text-lg leading-tight">{{ $pkg->name }}</h3>
              <p class="text-xs text-white/80 font-mono">{{ $pkg->name_en }}</p>
            </div>
          </div>
          ...
          <div class="p-6 text-right border-b border-gray-50">
            <div class="flex items-baseline justify-start gap-1">
              <span class="text-3xl font-black text-gray-900 tabular-nums">{{ number_format((float) str_replace(',', '', $pkg->price)) }}</span>
              <span class="text-xs font-bold text-gray-400">ريال</span>
              <span class="text-sm font-medium text-gray-400 mr-2">/ {{ $pkg->period }}</span>
            </div>
          </div>
        ...
        <div class="p-6 pt-0 text-right">
          <div class="flex items-center gap-2 pt-4 border-t border-gray-50">
            <button
              onclick="openEditModal({{ $pkg->id }}, '{{ addslashes($pkg->name) }}', '{{ addslashes($pkg->name_en) }}', '{{ str_replace(',', '', $pkg->price) }}', '{{ $pkg->period }}', '{{ $pkg->icon }}', {{ $pkg->popular ? 'true' : 'false' }}, '{{ $pkg->gradient }}', {{ $pkg->active ? 'true' : 'false' }}, {{ json_encode($pkg->features ?? []) }})"
            ...
            <form action="{{ route('admin.packages.destroy', $pkg->id) }}" method="POST" class="flex-1" onsubmit="return confirm('هل تريد حذف الباقة نهائياً؟');">
              @csrf
              @method('DELETE')
              <button
                type="submit"
  ...
  <div id="create-modal" class="hidden fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="relative bg-white rounded-3xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto animate-scale-in text-right">
      ...
      <form action="{{ route('admin.packages.store') }}" method="POST" class="p-6 space-y-4 text-gray-700">
        @csrf
