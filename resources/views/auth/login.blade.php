          ...
          <div class="inline-flex items-center gap-3 mb-6">
            <div class="w-14 h-14 bg-white rounded-2xl flex items-center justify-center border border-gray-100 shadow-2xl p-1.5 overflow-hidden">
              @if(!empty($settings->logo))
                ...
                @else
                  <span class="text-primary-600 text-xl font-bold">{{ $settings->logo }}</span>
                @endif
              @else
          ...
          @if($errors->any())
            <div class="mb-5 bg-red-50 border-r-4 border-red-500 rounded-xl p-4 flex items-start gap-3">
              ...
              <div>
                ...
                <p class="text-xs text-red-600 mt-1 leading-relaxed">{{ $errors->first() }}</p>
              </div>
            </div>
          @endif
          ...
          <form action="/login" method="POST" class="space-y-5">
            ...
            <div class="flex items-center justify-between text-xs">
              <label class="flex items-center gap-2 cursor-pointer text-gray-500 font-medium">
                <input type="checkbox" name="remember" class="rounded text-primary-600 focus:ring-primary-500" {{ old('remember') ? 'checked' : '' }} />
