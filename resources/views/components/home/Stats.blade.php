  ...
  <div class="max-w-7xl mx-auto px-4 relative">
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-10">
      @foreach($stats as $index => $stat)
        <div class="text-center group stat-card" data-target="{{ $stat['value'] }}" data-isk="{{ $stat['is_k'] ? 'true' : 'false' }}">
          <div class="w-18 h-18 bg-gradient-to-br {{ $stat['color'] }} rounded-2xl flex items-center justify-center mx-auto mb-5 shadow-lg group-hover:scale-110 group-hover:rotate-6 transition-all duration-300 w-[72px] h-[72px]">
            <i data-lucide="{{ $stat['icon'] }}" class="text-white w-8 h-8"></i>
          </div>
          <div class="text-4xl md:text-5xl font-black text-white mb-2 tabular-nums">
            <span class="count-value">0</span>
            <span class="text-emerald-400 font-bold">{{ $stat['suffix'] }}</span>
          </div>
          <div class="text-white/50 font-medium text-sm">{{ $stat['label'] }}</div>
        </div>
      @endforeach
    </div>
