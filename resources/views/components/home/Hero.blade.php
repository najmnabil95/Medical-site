  ...
  <div id="hero-slides-wrapper" class="absolute inset-0 z-0">
    @foreach($heroSlides as $index => $slide)
      ...
      >
        ...
        <div class="absolute inset-0 bg-[linear-gradient(270deg,rgba(248,250,252,0.95)_0%,rgba(248,250,252,0.8)_50%,rgba(248,250,252,0.2)_100%)]" style="opacity: {{ ($settings->hero_overlay_opacity ?? 80) / 100 }};"></div>
      </div>
    @endforeach
  </div>
        ...
        @foreach($carePath as $index => $item)
          <div class="rounded-[12px] border border-slate-100 bg-white/60 p-4 shadow-sm backdrop-blur-md">
            <div class="mb-4 flex items-center justify-between">
              <i data-lucide="{{ $item['icon'] }}" class="text-emerald-500 w-5 h-5"></i>
              <span class="font-mono text-xs text-slate-400">0{{ $index + 1 }}</span>
            </div>
    ...
    <div class="flex items-center gap-2">
      @foreach($heroSlides as $index => $slide)
        <button
          ...
          class="hero-dot-indicator h-2.5 rounded-full transition-all {{ $index === 0 ? 'w-8 bg-emerald-500' : 'w-2.5 bg-slate-300 hover:bg-slate-400' }}"
